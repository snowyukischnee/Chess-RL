...
class Board2State0(Board2State):
    ...
    def static_eval(board: chess.Board, actions: list) -> Any:
        """Convert the board to state (static version)
                :param
                    board(chess.Board): the chessboard class
                    actions(list): list all available action (including some illegal moves)
                :return:
        ...
        board_overview[3] = board.has_kingside_castling_rights(chess.BLACK)
        board_overview[4] = board.has_queenside_castling_rights(chess.BLACK)
        board_overview[5] = len(board.pieces(chess.PAWN, chess.WHITE))
        board_overview[6] = len(board.pieces(chess.ROOK, chess.WHITE))
        board_overview[7] = len(board.pieces(chess.KNIGHT, chess.WHITE))
        board_overview[8] = len(board.pieces(chess.BISHOP, chess.WHITE))
        board_overview[9] = len(board.pieces(chess.QUEEN, chess.WHITE))
        board_overview[10] = len(board.pieces(chess.KING, chess.WHITE))
        board_overview[11] = len(board.pieces(chess.PAWN, chess.BLACK))
        board_overview[12] = len(board.pieces(chess.ROOK, chess.BLACK))
        board_overview[13] = len(board.pieces(chess.KNIGHT, chess.BLACK))
        ...
        piece_positions[3][list(board.pieces(chess.BISHOP, chess.WHITE))] = 1
        piece_positions[4][list(board.pieces(chess.QUEEN, chess.WHITE))] = 1
        piece_positions[5][list(board.pieces(chess.KING, chess.WHITE))] = 1
        piece_positions[6][list(board.pieces(chess.PAWN, chess.BLACK))] = 1
        piece_positions[7][list(board.pieces(chess.ROOK, chess.BLACK))] = 1
        piece_positions[8][list(board.pieces(chess.KNIGHT, chess.BLACK))] = 1
